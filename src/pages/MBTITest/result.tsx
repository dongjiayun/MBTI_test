...
export default function Result() {
    ...
    const [videoAd,setVideoAd] = useState<any>()
    ...
    useLoad(()=>{
        ...
        if (Taro.createRewardedVideoAd) {
            const ad = Taro.createRewardedVideoAd({
                adUnitId: 'adunit-6c6d2bdc5526cd3a'
            })
            ad.onLoad(() => {
                console.log('广告加载完成')
            })
        ...
        <>
            <View className={styles.jbbMbti}>
                ...
                <View style={{marginTop:'40px'}}>
                    <Button  size='large' block type='primary' openType='share'>分享给好友</Button>
                </View>
                <Divider>{characterData?.subtitle}</Divider>
                {
                    !hasWatchedAd &&
                    <View style={{margin:'40px 0'}}>
                        <Button  size='large' block  onClick={handleWatchAd}>观看广告查看详细性格分析</Button>
                    </View>
                }
                <View className={!hasWatchedAd&&styles.hiddenResult}>
            ...
            <canvas canvas-id='myCanvas' style={{position: 'absolute', left: '-9999px'}}></canvas>
        </>
    )
}
